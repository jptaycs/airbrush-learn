...
- [ ] **Fix `Blocked - Not Published` never updating the topics sheet.** Root-caused on 2026-08-17 by tracing the actual node connections in the exported workflow ("SEO Content Creation - Multi-Agent Pipeline (v3 - Jerome)"): `Filter - Unpublished Only` keeps any topic where `Status != "Published"`. If any of the Orchestrator Agent's six sub-agents fails, it returns `PIPELINE_FAILED` per its own system prompt, `Publish Guard` correctly detects this and routes to `Blocked - Not Published` — but that node **has no outgoing connection at all**. It sets an in-memory status/reason and dead-ends; nothing is ever written back to the sheet. Since the sheet's `Status` never changes, the same topic gets handed back on every subsequent run forever, with no visible failure signal anywhere except n8n's own execution history. This is what caused the "How to Airbrush Freehand" topic to be reprocessed/overwritten multiple times (duplicates from an earlier occurrence cleaned up in commit `d114f41`) and what caused an August 2026 run to produce nothing at all, anywhere. The earlier theory that the `Update row in sheet` node's row-matching (`" #"` column) was the culprit was a red herring — that node is only reached on the *success* path (after a real WordPress post is created) and looked correctly configured **as of 2026-08-17, before the second failure mode below was found**. **The actual fix:** give `Blocked - Not Published` an outgoing connection that writes the block status + reason back to the sheet row (and ideally alerts someone), so failed topics stop looping silently. See the `n8n-pipeline-architecture` and `n8n-topics-sheet` memory files for the full node-by-node trace and the sheet's URL/columns.

- [ ] **Sheet also never updates on a *successful* run — same symptom, different node, much bigger impact.** Confirmed 2026-08-18: the "Createx vs Badger Airbrush Paint" topic was committed to `articles.json` and published to the live site twice (`3aedce2` on 2026-08-17, `63a2f7c` on 2026-08-18), yet its sheet row's `Status` is still `"Not started"` — not `"Published"`, not even `"Blocked"`. This turned out to be the norm, not a one-off: the repo's 6 live articles are really only **~4 distinct topics**, each reprocessed and recommitted 2-10 times (`git log -- src/data/articles.json` shows the "freehand airbrushing" topic alone was regenerated across 3 different slugs in 10 separate commits). **Hypothesis, not yet re-confirmed against a fresh n8n export:** `Update row in sheet` is the *only* node in the workflow that writes back to the sheet, and it lives exclusively on the WordPress-posting branch (`Upload Featured Media to WordPress` → `Create WordPress Draft Post` → `Update row in sheet`), which forks in parallel from `Generate Featured Image` alongside — but fully independent of — the GitHub-commit branch that actually publishes content today. Since this site fully replaced WordPress, if that branch is now broken/disabled/pointing nowhere, it fails or no-ops before ever reaching `Update row in sheet` — while the GitHub branch keeps succeeding every time regardless. Net effect: every successful publish looks like a silent failure to the sheet, so the same handful of topics loop forever and the other ~40 "Not started" rows never get reached. **To confirm:** check n8n's execution history for the WordPress-branch nodes on a recent successful run. **Likely fix once confirmed:** either move `Update row in sheet` onto the GitHub-commit branch (next to `Commit articles.json`/`Commit Hero Image`) and drop the dead WordPress branch, or repair whatever's broken in it if WordPress posting is still wanted. See the `n8n-pipeline-architecture` memory file for the full commit-history evidence.
