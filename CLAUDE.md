...
- [ ] **`/admin`'s Topics tab needs the same real-GitHub-round-trip verification the Articles and Gallery tabs already got.** `topics-{list,save,delete}.js` (added 2026-08-18) follow the exact same pattern as `admin-{list,save,delete}.js`, which was verified against real GitHub write access earlier — but the topics-specific functions themselves haven't been separately exercised against a live `GITHUB_PAT` yet. Test add/edit/delete on a real topic before trusting it, same as the standing caveat on the other admin functions below.